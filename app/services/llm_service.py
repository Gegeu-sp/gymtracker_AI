...
# Modelo local via Ollama (ex: "ollama pull qwen2.5:3b"), usado como fallback quando a Groq
# atinge o limite diário de tokens (429), e como primeira opção nas tarefas leves (ver
# structure_reference_table) para reduzir o consumo do orçamento da Groq.
...
class LLMRateLimitError(Exception):
    ...
    (HTTP 429) E o fallback local via Ollama também não conseguiu responder. Os routers
    capturam isso especificamente para devolver uma mensagem clara ao usuário em vez do
    500 genérico "Falha ao gerar treinos".
    """
    pass
...
def _build_rate_limit_message(e: Exception, ollama_error: Optional[str] = None) -> str:
    retry_seconds = None
    ...
    retry_hint = f"Tente novamente em ~{_format_duration(retry_seconds)}." if retry_seconds else "Tente novamente em alguns minutos."
    base = f"⏳ Limite diário de tokens da IA (Groq) atingido. {retry_hint}"

    if OLLAMA_AVAILABLE:
        return (f"{base} O fallback local (Ollama/{OLLAMA_MODEL}) também não respondeu "
                f"({ollama_error or 'motivo desconhecido'}) — confira se o Ollama está rodando "
                f"(`ollama serve`) ou aguarde o limite da Groq liberar.")
    return f"{base} Instale e rode o Ollama localmente para ter um fallback offline nesse tipo de erro."
...
def parse_workout_from_text(raw_text: str) -> dict:
    user_prompt = f"Extraia o treino da imagem/texto seguindo o mapeamento JSON (nickname=Intensidade/RPE, method=Cadência). Números únicos:\n\n{raw_text}"
    try:
...
def structure_reference_table(raw_text: str) -> list:
    ...
    É uma tarefa de limpeza/reorganização de texto, não exige o raciocínio do modelo principal
    — por isso tenta o Ollama local PRIMEIRO (custo zero de tokens da Groq) e só usa o
    LIGHT_MODEL da Groq como fallback se o Ollama não estiver disponível/rodando. Cacheada
    (@lru_cache): reenviar a mesma imagem/texto (ex: usuário tenta de novo após um erro) não
    gasta tokens/processamento de novo.
    """
